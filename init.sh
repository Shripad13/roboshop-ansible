...
if [ -z "$1" -o -z "$2" -o -z "$3"]; then
    ...
    echo "Expected Usage: \n\t\t bash init.sh frontend dev PWD vault_token"
    exit 89
fi
...
ansible-playbook -i ${COMPONENT}-${ENV}.devsecopswithshri.site, -e ansible_user=ec2-user -e ansible_password=${SSH_PASSWORD} -e COMPONENT=${COMPONENT} -e ENV=${ENV} -e vault_token=${vault_token} expense.yml
